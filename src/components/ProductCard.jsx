...
export default function ProductCard({ data }) {
    return (
        <div className="flex flex-col justify-center  bg-white shadow-lg rounded-lg p-5 gap-1 max-w-96">
            <Image src={data.image && data.image} alt={data.title} width={300} height={300} />
            <label className="flex justify-center items-center text-lg md:text-xl font-semibold">{data && data.title}</label>
            <label className="flex justify-center items-center text-lg md:text-xl font-semibold">{data && formatCurrency(data.price)}</label>
